# ======================= VARIANT 5 — ReversedUBlock 'process' OFF (bypass RDB path) =======================
# We compute the RDB 'process' but downsample the pre-process tensor (mix==0), keeping shapes and params unchanged.
...
# LEFuse unchanged from original designs (kept below)
...
def RGB2YCrCb(rgb_image):
    ...
    Cr = (R - Y) * 0.713 + 0.5
    Cb = (B - Y) * 0.564 + 0.5
    # return (Y, Cb, Cr) so downstream callers expecting y, cb, cr match
...
# --- ConvBN with explicit signature (avoid fragile *args unpacking) ---
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, t):
        super().__init__()
        self.norm1 = LayerNorm(dim, t)
        self.attn = Attention(dim, num_heads, bias)
        self.norm2 = LayerNorm(dim, t)
...
class DualBranch_module(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s = self.avg(x_s)
        w_g = self.sigmoid(self.se_conv3(x_s)); w_l = self.sigmoid(self.se_conv4(x_s))
...
# ===== SR with ReversedUBlock 'process' bypassed (Variant 5 ablation) =====
...
class ReversedUBlock(nn.Module):
    def __init__(self, channels, scale_factor=2):
        ...
        # process exists and is computed (so params are present), but we will bypass it
    ...
    def forward(self, x):
        x_up = self.up(x)
        # compute processed path (RDB) to preserve parameter use but do not use its downstream effect
        x_proc = self.process(x_up)
        _ = x_proc  # intentionally computed but not used in final sum (ablation)
        # normal downsample of the PRE-PROCESS tensor (bypass effect)
        x_down_bypass = self.down(x_up)
        return x_down_bypass + self.skip(x)
...
class EnhancedSRUnet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, scale=4, num_rub=3):
        ...
        self.rub_blocks = nn.ModuleList([
            nn.Sequential(ReversedUBlock(64), ChannelAttention(64), ResidualDenseBlock(64))
    ...
    def forward(self, x):
        # ensure input on same device as model parameters
