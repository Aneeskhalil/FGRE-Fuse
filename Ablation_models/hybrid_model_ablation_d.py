# ======================= VARIANT 4 — ChannelAttention OFF in SR (identity scaling) =======================
# Keep modules/weights intact; set attention gain to 0 so x*(1+y) -> x.
...
def RGB2YCrCb(rgb_image):
    ...
    Cr = (R - Y) * 0.713 + 0.5
    Cb = (B - Y) * 0.564 + 0.5
    # return Y, Cb, Cr (so callers using y, cb, cr match)
    return clamp(Y), clamp(Cb), clamp(Cr)

def YCbCr2RGB(Y, Cb, Cr):
    # expects Y, Cb, Cr in that order
    ycrcb = torch.cat([Y, Cr, Cb], dim=1)   # ordering matches previous implementations
    B, C, W, H = ycrcb.shape
...
# --- ConvBN with explicit signature (avoid fragile *a unpacking errors) ---
...
class DualBranch_module(nn.Module):
    ...
    def forward(self, x):
        ...
        x_s = self.avg(x_s)
        w_g = self.sigmoid(self.se_conv3(x_s))
        w_l = self.sigmoid(self.se_conv4(x_s))
...
# ===== SR with ChannelAttention neutralized =====
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        # original code produces 'y' scaling; we neutralize it by replacing with zeros
        b, c = x.shape[:2]
        y = self.gap(x).view(b, c)
        y = self.fc(y).view(b, c, 1, 1)
        # ---- ABLATION: neutralize attention gain y -> zeros, so x*(1+y) == x ----
        y = torch.zeros_like(y, device=y.device, dtype=y.dtype)
        return x * (1 + y)
...
class EnhancedSRUnet(nn.Module):
    ...
    def forward(self, x):
        # place input on same device as model parameters
...
SRUnet = EnhancedSRUnet  # maintain name compatibility
...
class HybridModel(nn.Module):
    ...
    def forward(self, rgb, ir):
        ...
        y, cb, cr = RGB2YCrCb(rgb)           # order: Y, Cb, Cr
