# ======================= VARIANT 2 — Local (CNN) contribution OFF in LEFuse =======================
# Same parameters and names as your working file; only the Local branch contribution is nulled.
import torch
...
class DualBranch_module(nn.Module):
    ...
    def forward(self, x):
        res = x
        x_g = self.Global(x)
        x_l = self.Local(x)            # computed but will be nulled in final sum
        ...
        # ---- ABLATION (Variant 2): remove local contribution in the residual sum ----
        return res + (x_l * w_l * 0.0) + (x_g * w_g)
