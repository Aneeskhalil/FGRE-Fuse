# ======================= VARIANT 1 — Global (Transformer) contribution OFF in LEFuse =======================
# Same parameters as your file; we still compute the Global branch but its contribution to the output is nulled.
import torch
...
def RGB2YCrCb(rgb_image):
    ...
    Cr = (R - Y) * 0.713 + 0.5
    Cb = (B - Y) * 0.564 + 0.5
    Y = clamp(Y); Cr = clamp(Cr); Cb = clamp(Cb)
    return Y, Cr, Cb
...
class DualBranch_module(nn.Module):
    ...
    def forward(self, x):
        res = x
        x_g = self.Global(x)           # computed but will be nulled in final sum
        x_l = self.Local(x)
        ...
        # ---- ABLATION: remove global contribution in the residual sum ----
        return res + (x_l * w_l) + (x_g * w_g * 0.0)

class up_block(nn.Module):
    def __init__(self, dim_in, dim_out):
        super().__init__()
        self.up = nn.Sequential(nn.ConvTranspose2d(dim_in, dim_out, 4, 2, 1, bias=False), nn.ReLU())
...
# ===== SR (unchanged definitions; ablation is only in LEFuse here) =====
...
class EnhancedSRUnet(nn.Module):
    def __init__(self, in_channels=3, out_channels=3, scale=4, num_rub=3):
        ...
        self.upsample = nn.Sequential(
            nn.Conv2d(64, 64*(scale**2), 3, padding=1), nn.PixelShuffle(scale),
            nn.Conv2d(64, out_channels, 3, padding=1)
        )
        self.refine = nn.Sequential(
            nn.Conv2d(out_channels, 32, 3, padding=1), nn.LeakyReLU(0.2, inplace=True),
    ...
    def forward(self, x):
        ...
        x0 = self.head(x)
        feats = [x0]
        for rub in self.rub_blocks: feats.append(rub(feats[-1]))
...
class HybridModel(nn.Module):
    ...
    def forward(self, rgb, ir):
        ...
        fused_rgb_hr = self.unet(fused_rgb_lr)
        return fused_rgb_hr
